...
class ProjectUpdateTest(TestCase):
    ...
    def test_authenticate_update_project(self):
        ...
        response = self.client.post(reverse('updateproject', kwargs={'pk': self.project.id}),
                                    {'project_title': 'abcd',
                                     'description': 'abcd1',
                                     'assigned_to': [self.member1.id, self.member.id]})
...
class ProjectSettingsTest(TestCase):
    ...
    def setUp(self):
        self.instance = N(MyUser)
        self.member = N(MyUser)
        self.password = self.instance.password
        self.instance.password = make_password(self.password)
        self.instance.save()
        self.password1 = self.member.password
        self.member.password = make_password(self.password1)
        self.member.save()

        self.project = G(Project, project_manager=self.instance, assigned_to=[self.member, self.instance])
        self.project2 = G(Project,project_manager=self.member, assigned_to=[self.instance])
        self.story = G(Story, project_title=self.project, email=self.instance)
        return self.instance
    ...
    def test_authenticated_user_project_setting(self):
        self.client.login(email=self.member.email, password=self.password1)
        response = self.client.get(reverse('project_settings', kwargs={'pk': self.project2.id}))
        self.assertEqual(response['content-type'], 'text/html; charset=utf-8')
        self.assertEqual(response.status_code, 200)
    ...
    def test_authenticated_user_search_story_mail(self):
        self.client.login(email=self.instance.email, password=self.password)
        kwargs={'HTTP_X_REQUESTED_WITH':'XMLHttpRequest'}
        response = self.client.get(reverse('project_settings', kwargs={'pk':self.project.id}), {'initial_date':'2015-07-27','final_date':'2015-07-29', 'mail':True}, **kwargs)
        self.assertEqual(response['content-type'], 'html')
        self.assertEqual(response.status_code,200)
